...
sleep :: Typeable a => TimeTDiff -> Stream a -> Stream ()
...
runMode :: Typeable a => TimeTDiff -> Stream a -> Stream RunMode
...
costJumps ct wakeup = let
  ...
  getTimeT (T x) _ = x
  getTimeT _ y = y
  timediff = round.realToFrac <$> (diffUTCTime <$> now <*> prevt)
  ---
